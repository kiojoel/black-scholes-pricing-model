...
tab1, tab2 = st.tabs(["Single Option Pricer", "Portfolio Manager"])
...
with tab2:
    st.header("💼 Portfolio Risk Manager")
    st.write("Manage your portfolios and analyze your aggregate risk exposure.")
    ...
    with st.expander("➕ Create New Portfolio"):
        ...
        if st.button("Create Portfolio"):
            if new_portfolio_name:
                try:
                    create_portfolio(new_portfolio_name)
    ...
    if not portfolios:
        st.info("No portfolios found. Create one using the form above.")
    else:
        portfolio_names = [p['name'] for p in portfolios]
        selected_portfolio_name = st.selectbox("Select a Portfolio to View", portfolio_names)
        selected_portfolio = next((p for p in portfolios if p['name'] == selected_portfolio_name), None)

        if selected_portfolio:
            # --- Add New Position Form ---
            with st.expander("➕ Add a New Position"):
                with st.form("new_position_form", clear_on_submit=True):
                    c1, c2, c3, c4 = st.columns(4)
                    with c1:
                        pos_ticker = st.text_input("Ticker")
                        pos_asset_type = st.selectbox("Asset Type", ["stock", "call", "put"])
                    with c2:
                        pos_quantity = st.number_input("Quantity", step=1)
                        pos_strike = st.number_input("Strike Price", format="%.2f", disabled=(pos_asset_type == 'stock'))
                    with c3:
                        pos_exp_date = st.date_input("Expiration", disabled=(pos_asset_type == 'stock'))
                    with c4:
                        st.write("") # Spacer
                        st.write("") # Spacer
                    ...
                    if submitted:
                        if not pos_ticker or pos_quantity == 0:
                            st.warning("Please provide a Ticker and a non-zero Quantity.")
                        else:
                            exp_date_str = pos_exp_date.strftime('%Y-%m-%d') if pos_asset_type != 'stock' else None
                            try:
                                add_position(
                                    ...
                                    strike_price=pos_strike if pos_asset_type != 'stock' else None,
                                    expiration_date=exp_date_str
                                )
                                st.success(f"Added {pos_quantity} {pos_ticker.upper()} {pos_asset_type} to '{selected_portfolio_name}'")
                                st.rerun()
                            except Exception as e:
                                st.error(f"Error adding position: {e}")

            # --- Display Current Positions ---
            st.subheader(f"Positions in '{selected_portfolio_name}'")
            positions = get_positions_for_portfolio(selected_portfolio['id'])

            if positions:
                positions_df = pd.DataFrame(positions)
                # Only drop columns if they exist
                columns_to_drop = [col for col in ['id', 'portfolio_id'] if col in positions_df.columns]
                if columns_to_drop:
                    positions_df = positions_df.drop(columns=columns_to_drop)
                st.dataframe(positions_df, use_container_width=True)
            else:
                st.info("This portfolio has no positions yet. Add one using the form above.")
